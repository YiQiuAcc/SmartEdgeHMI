...
public class AlarmStateMachine : IAlarmStateMachine
{
    ...
    public void Acknowledge(long alarmId)
    {
        lock (_syncRoot)
        {
            var entry = _activeAlarms.FirstOrDefault(kv => kv.Value.Record.Id == alarmId);
            if (entry.Value is null) return;

            ApplyAck(entry.Key, entry.Value);
        }
        AlarmStatesChanged?.Invoke();
    }
    ...
    private void ApplyAck(string deviceId, AlarmContext ctx)
    {
        switch (ctx.Record.State)
        {
            case AlarmState.UNACK:
